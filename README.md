...
def gen_pdf(code_list):
    ...
    class PDF(FPDF):
        ...
        def addInfo(self,codeText, codeN,s1,s2,w,h):
            self.rect(s1, s2, w,h)
            img=qrcode.make(codeText)
            img.save("qrCodes/"+codeText+".png")
            
            self.image("qrCodes/"+codeText+".png", x=s1 +1.5,y=s2+1.5,link='', type='', w=45, h=45)

            self.set_font('Arial', '', 7)
            self.text(x=s1+13, y=s2+4, txt=codeText)
            self.text(x=s1+48, y=s2+15, txt='Nome')
            self.text(x=s1+48, y=s2+25, txt='Cognome')
            self.text(x=s1+48, y=s2+35, txt='Numero tessera' + codeN)

            self.text(x=s1+48, y=s2+45, txt='Oratorio Paolo VI')
    ...
    s1=8.0 # mMargin from start of sheet  
    s2=8.0 # Margin from top of sheet
    w=93 # Width of each QR code
    h=48 # Height of each QR code

    for item in code_list:
    
        pdf.addInfo(item[0], item[1],s1,s2,w,h)
        s1=s1+w+8.0
                
        itemCount+=1
        # As each row can have 2 items
        if(itemCount%2==0):
            s1=8.0
            s2=s2+h+4.0
